...
**Lignes concernées** : 18, 50, 52-53, 103

```csharp
var clients = LoadClientsFromDatabase(connectionString);  // Ligne 18 - Appel synchrone
connection.Open();                                         // Ligne 50 - Bloquant
var reader = command.ExecuteReader()                       // Ligne 52 - Bloquant
smtp.Send(mail);                                          // Ligne 103 - Bloquant
```
...
**Lignes concernées** : 48-71 (fonction `LoadClientsFromDatabase`), 95-104 (fonction `SendNotificationEmail`)
...
**Lignes concernées** : 50, 99-102

```csharp
using (var connection = new SqlConnection(connectionString))  // Ligne 50
var smtp = new SmtpClient("smtp.validflow.com", 587)         // Ligne 99
```
